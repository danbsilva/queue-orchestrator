...
def create_token(payload):
    ...
    if not token:
        Logger().dispatch('INFO', __module_name__, 'create_token', messages.ERR_TOKEN_GENERATE)
...
def verify_token(f):
    ...
    def decorated(*args, **kwargs):
        try:
            ...
            Logger().dispatch('INFO', __module_name__, 'verify_token', messages.TOKEN_IS_MISSING)
            return {'message': messages.TOKEN_IS_MISSING}, 401

        if not token.startswith('Bearer '):
            Logger().dispatch('INFO', __module_name__, 'verify_token',  messages.INVALID_TOKEN_FORMAT)
        ...
        if cache_token:
            Logger().dispatch('INFO', __module_name__, 'verify_token', messages.TOKEN_IS_INVALID)
        ...
        try:
            ...
            user_db = UserModel.get_by_uuid(user_authenticated['uuid'])
            if not user_db:
                Logger().dispatch('INFO', __module_name__, 'verify_token',messages.UNAUTHORIZED_USER)
...
def verify_token_email(token):
    try:
        ...
        Logger().dispatch('INFO', __module_name__, 'verify_token_email', messages.TOKEN_IS_INVALID)
        return {'message': messages.TOKEN_IS_INVALID}, 401
...
def admin_required(f):
    ...
    def decorated(*args, **kwargs):
        ...
        user_db = UserModel.get_by_uuid(user_authenticated['uuid'])
        if not user_db:
            Logger().dispatch('INFO', __module_name__, 'admin_required', messages.UNAUTHORIZED_USER)
            return {'message': messages.UNAUTHORIZED_USER}, 401
