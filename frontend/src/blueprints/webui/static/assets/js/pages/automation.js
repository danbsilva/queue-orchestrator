...
function loadPageAutomations(page, per_page, search=''){
    $.ajax({
        type: "GET",
        url: Flask.url_for('automations.page_automations', {"page": page, "per_page": per_page, "search": search}),
        beforeSend: function () {
        },
...
// Load Pages Owners
function loadPageOwners(automation_uuid, page, per_page, search=''){
    $.ajax({
        type: "GET",
        url: Flask.url_for('automations.page_owners', {"automation_uuid": automation_uuid, "page": page, "per_page": per_page, "search": search}),
        beforeSend: function () {
        },
        success: function(data) {
            $("#owners").html(data)
...
// Load Pages Steps
function loadPageSteps(automation_uuid, page, per_page, search=''){
    $.ajax({
        type: "GET",
        url: Flask.url_for('automations.page_steps', {"automation_uuid": automation_uuid, "page": page, "per_page": per_page, "search": search}),
        beforeSend: function () {
        },
        success: function(data) {
            $("#steps").html(data)
...
// Load Pages Fields
function loadPageFields(step_uuid, page, per_page, search=''){
    $.ajax({
        type: "GET",
        url: Flask.url_for('automations.page_fields', {"step_uuid": step_uuid, "page": page, "per_page": per_page, "search": search}),
        beforeSend: function () {
        },
        success: function(data) {
            $("#fields").html(data);
...
function loadPageItemsByStep(step_uuid, page, per_page, search=''){
    $.ajax({
        type: "GET",
        url: Flask.url_for('automations.page_items', {"step_uuid": step_uuid, "page": page, "per_page": per_page, "search": search}),
        beforeSend: function () {
        },
        success: function(data) {
            $("#items").html(data)
...
$(document).ready(function () {
    ...
    $('.layout-page').on('keyup', "#automation-search", function(e) {
        ...
        var search = $(this).val();
        // Verificar se a pesquisa tem 3 ou mais caracteres
        if (search.length >= 3) {
            loadPageAutomations(page, per_page, search);
    ...
    // Per Page
    $('.layout-page').on('change',"select#owner-per-page", function(e){
        e.preventDefault();
        var automation_uuid = $(this).attr('data-automation');
        var per_page = $(this).children("option:selected").val();
    ...
    // Pagination
    $('.layout-page').on('click', '.a-page-owner', function(e){
        e.preventDefault();
        var automation_uuid = $(this).attr('data-automation');
        var page = $(this).attr('id');
        var per_page = $('select#owner-per-page').children("option:selected").val();
        loadPageOwners(automation_uuid, page, per_page);
    });

    // Search
    $('.layout-page').on('keyup', "#owner-search", function(e) {
        e.preventDefault();
        var search = $(this).val();
        var automation_uuid = $(this).attr('data-automation');
        var page = $('.layout-page .page-item.active a').attr("id");
        ...
        // Verificar se a pesquisa tem 3 ou mais caracteres
        if (search.length >= 3) {
            loadPageOwners(automation_uuid, page, per_page, search);
        }else if (search.length == 0) {
            loadPageOwners(automation_uuid, page, per_page);
        }
    });
    ...
    // Add Owner
    $('.layout-page').on('click', '.addowner', function(e){
        e.preventDefault();
        var automation_uuid = $(this).attr('data-automation');
        $.ajax({
          type: "GET",
          url: Flask.url_for('automations.new_owner', {"automation_uuid": automation_uuid}),
          beforeSend: function () {
            $('#modalForm .modal-body').html('');
            $('#modalForm .modal-title').html('NOVO PROPRIETÁRIO');
          },
          success: function(data) {
            $('#modalForm .modal-body').html(data);
            $('#modalForm').modal('show');
    ...
    $('.layout-page').on('submit', "#NewOwnerForm", function(e) {
        e.preventDefault();
        var formData = $(this).serialize();
        var automation_uuid = $(this).attr('data-automation');
        var page = $('.layout-page .page-item.active a').attr("id");
        ...
        $.ajax({
          ...
          success: function(data) {
            $('#modalForm').modal('hide');
            loadPageOwners(automation_uuid, 1, per_page);
            sendNotification(data, 'success');
          },
          complete: function () {
          },
    ...
    // Delete Owner
    $('.layout-page').on('click', '.deleteowner', function(e){
        e.preventDefault();
        ...
        $.ajax({
          type: "GET",
          url: Flask.url_for('automations.delete_owners', {"automation_uuid": automation_uuid, "uuid": uuid, "name": name, "email": email}),
          beforeSend: function () {
          },
          success: function(data) {
            loadPageOwners(automation_uuid, page, per_page);
    ...
    $('.layout-page').on('keyup', "#step-search", function(e) {
        ...
        // Verificar se a pesquisa tem 3 ou mais caracteres
        if (search.length >= 3) {
            loadPageSteps(automation_uuid, page, per_page, search);
        }else if (search.length == 0) {
            loadPageSteps(automation_uuid, page, per_page);
        }
    });
    ...
    $('.layout-page').on('keyup', "#field-search", function(e) {
        ...
        // Verificar se a pesquisa tem 3 ou mais caracteres
        if (search.length >= 3) {
            loadPageFields(step_uuid, page, per_page, search);
        }else if (search.length == 0) {
            loadPageFields(step_uuid, page, per_page);
        }
    });
    ...
    $('.layout-page').on('keyup', "#item-search", function(e) {
        ...
        // Verificar se a pesquisa tem 3 ou mais caracteres
        if (search.length >= 3) {
            loadPageItemsByStep(step_uuid, page, per_page, search);
        }else if (search.length == 0) {
