    ...
    function L(a, b, c) {
        if (b.enablePagination) {
            var d = '<{0} class="actions {1}"><ul role="menu" aria-label="{2}">{3}</ul></{0}>'
                  , e = '<li><a href="#{0}" role="menuitem" class="{1}">{2}</a></li>'
                  , f = "";

            b.forceMoveForward || (f += e.format("previous", "bg-primary", b.labels.previous)),
            f += e.format("next", "bg-primary", b.labels.next),
            b.enableFinishButton && (f += e.format("finish","bg-primary", b.labels.finish)),
            b.enableCancelButton && (f += e.format("cancel", "bg-danger", b.labels.cancel)),
